...
public final class LanguageManager<T extends WrappedConfig & ILanguage> {
    ...
    private void reloadLanguageSettings() {
        ...
        this.forceDefaultLanguage = config.enforceDefaultLocale();
        this.debugLanguage = config.debugMode();
        this.usePlaceholderAPI = config.usePlaceholderAPI() && FabricLoader.getInstance().isModLoaded("placeholder-api");
    }
    ...
    private @NotNull Component replacePlaceholderAPI(@Nullable CommandSourceStack audience, @NotNull Component message) {
        ...
        return audiences.toAdventure(TextPlaceholderAPIIntegration.format(player, audiences.toNative(message)));
    }
    ...
    public void sendMessage(@NotNull CommandSourceStack audience, @NotNull Component message) {
        if (!Component.empty().equals(message)) audience.sendMessage(message);
    }
    ...
    public void sendActionBar(@NotNull CommandSourceStack audience, @NotNull Component message) {
        if (!Component.empty().equals(message)) audience.sendActionBar(message);
    }

    public void broadcastMessage(@NotNull MinecraftServer server, @NotNull String path, @NotNull ConfigGetter<T, String> value, @Nullable String permission, @NotNull Replacer... replacers) {
        List<ServerPlayer> players = server.getPlayerList().getPlayers();
        this.broadcastMessage(server, players, path, value, permission, replacers);
    }

    public void broadcastMessage(@NotNull MinecraftServer server, @NotNull Iterable<? extends ServerPlayer> players, @NotNull String path, @NotNull ConfigGetter<T, String> value, @Nullable String permission, @NotNull Replacer... replacers) {
    ...
    private boolean hasPermission(@NotNull ServerPlayer player, @Nullable String permission) {
        return permission == null || permission.isEmpty() || Permissions.check(player, permission);
    }
